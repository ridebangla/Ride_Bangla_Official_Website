...
import { getWebsiteUpdates } from "@/lib/website-data";
...
export function Header() {
  ...
  const { data: notifications } = useQuery({
    ...
    retry: false,
    staleTime: 0,
    queryFn: async () => (await getWebsiteUpdates(5)) as UpdateNotification[],
  });
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border bg-background/95 shadow-sm backdrop-blur supports-[backdrop-filter]:bg-background/80">
      <div className="mx-auto flex h-16 w-full max-w-6xl items-center justify-between px-4">
        ...
        >
          ...
          <div className="min-w-0">
            ...
            <span className="hidden text-[10px] font-semibold uppercase tracking-wider text-brand-green sm:block">
              Digital Ecosystem
            </span>
          </div>
        </Link>
        ...
        <nav className="hidden items-center gap-1 md:flex" aria-label="Main">
          {nav.map((n) => (
            <Link
              ...
              className="rounded-md px-3 py-2 text-sm font-medium text-foreground/80 transition hover:bg-secondary hover:text-brand-green-dark"
              activeProps={{
                className:
        ...
        <div className="flex items-center gap-1">
          <div className="relative">
            <button
              ...
              className="relative inline-flex h-10 w-10 items-center justify-center rounded-md transition hover:bg-secondary"
          ...
          <button
            className="inline-flex h-10 w-10 items-center justify-center rounded-md transition hover:bg-secondary md:hidden"
            onClick={() => {
              setOpen((v) => !v);
              setNotificationOpen(false);
            }}
      ...
      {open ? (
        <nav
          className="border-t border-border bg-background shadow-sm md:hidden"
          aria-label="Mobile"
        >
            ...
            {nav.map((n) => (
              <li key={n.to}>
                <Link
                  ...
                  className="block rounded-md px-3 py-2 text-base font-medium text-foreground transition hover:bg-secondary"
                  activeProps={{
                    className:
