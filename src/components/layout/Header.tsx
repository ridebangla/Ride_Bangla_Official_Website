        ...
        <div className="flex h-[68px] items-center justify-between px-4 sm:px-6">
          <Link to="/" className="flex items-center gap-3" aria-label="Ride Bangla home" onClick={() => setOpen(false)}>
            ...
            <div className="leading-none">
              <span className="block text-lg font-black tracking-tight text-[#06291f]">Ride <span className="text-brand-green">Bangla</span> <span className="text-sm font-extrabold text-brand-red">Limited</span></span>
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 xl:hidden" onClick={() => { setOpen((value) => !value); setNotificationOpen(false); setSearchOpen(false); }} aria-label="Toggle menu">{open ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}</button>
          </div>
        </div>
