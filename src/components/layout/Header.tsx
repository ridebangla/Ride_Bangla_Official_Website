...
export function Header({ overlay = false }: { overlay?: boolean }) {
  ...
  return (
    <header className={`${overlay ? "absolute left-0 right-0 top-0 z-50 px-3 pt-3 sm:px-5" : "sticky top-0 z-50 px-3 pt-3 sm:px-5"}`}>
      <div className={`mx-auto max-w-7xl rounded-2xl border border-slate-200/70 bg-white/95 shadow-[0_15px_50px_rgba(0,45,30,.10)] backdrop-blur-xl ${overlay ? "lg:rounded-[1.8rem]" : ""}`}>
        <div className="flex h-[68px] items-center justify-between px-4 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className={overlay ? "hidden" : "relative"}>
