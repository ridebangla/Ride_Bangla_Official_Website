  ...
  head: () => ({
    meta: [
      { title: "Help Center — Ride Bangla" },
      {
        name: "description",
        content:
          "Frequently asked questions and support resources for Ride Bangla customers, riders and partners.",
      },
    ],
...
function HelpCenterPage() {
  ...
  return (
    <SiteLayout>
      <PageHeader
        title="Help Center"
        subtitle="Search our FAQs or reach out — our support team is here for you."
      />
      ...
      <section className="mx-auto max-w-4xl px-4 py-10">
        ...
        <div className="mt-6 space-y-3">
          {filtered.length === 0 ? (
            ...
            filtered.map((faq) => (
              <details
                key={faq.id}
                className="group rounded-xl border border-border bg-card p-4 open:shadow-sm"
              >
        ...
        <div className="mt-12 rounded-2xl border border-brand-green/30 bg-brand-green-soft p-6 text-center sm:p-8">
          ...
          <p className="mx-auto mt-2 max-w-xl text-sm text-muted-foreground">
            Our support team is happy to answer your questions and help with any
            issues.
          </p>
...
function AiAssistant() {
  ...
  const onAsk = async (event: React.FormEvent) => {
    ...
    try {
      const response = await ask({ data: { question: q } });
    ...
    <div className="mb-8 rounded-2xl border border-brand-green/30 bg-gradient-to-br from-brand-green-soft to-background p-5 shadow-sm sm:p-6">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Get instant answers to common questions. For account or order help,
            contact Support.
          </p>
        </div>
      </div>
      ...
      {answer && (
        <div className="mt-4 rounded-lg border border-border bg-card p-4 text-sm text-foreground">
          <div className="mb-1 text-xs font-bold uppercase tracking-wider text-brand-green">
            AI Answer
          </div>
