...
const testimonials = [
  {
    ...
    text: "Ride Bangla delivers on time and the service is excellent. I always trust them for my food and parcel delivery.",
  },
  {
    name: "Md. Rafiq Hasan",
    text: "Best delivery service in Bangladesh! The riders are professional and the app is very easy to use.",
  },
  {
    name: "Sadia Akter",
    text: "I love that they have IT services too. Especially their women-first initiative is truly inspiring.",
  },
  {
    name: "Tanjim Ahmed",
    text: "Fast, safe and reliable. Ride Bangla is now my go-to service for everything.",
  },
];
  ...
  return (
    <SiteLayout>
      <div className="rb-home overflow-hidden">
        {/* HERO */}
        <section
          className="relative isolate min-h-[720px] bg-[#002c20] text-white lg:min-h-[780px]"
        ...
        >
          ...
          <div
            className="absolute inset-y-0 right-0 w-full bg-cover bg-center opacity-80 lg:w-[68%]"
        ...
        <section className="px-5 py-20 sm:px-8 lg:py-28">
          <div className="mx-auto grid max-w-7xl items-center gap-12 rounded-[2.5rem] bg-gradient-to-br from-[#e9f8f0] via-white to-[#fff4f3] p-6 sm:p-10 lg:grid-cols-[.8fr_1.2fr] lg:p-14">
            <div className="relative mx-auto w-full max-w-sm">
              <div className="absolute inset-8 rounded-full bg-emerald-300/25 blur-3xl" />
              <div className="relative mx-auto w-56 rounded-[3rem] border-[8px] border-[#082d22] bg-[#082d22] p-2 shadow-[0_30px_70px_rgba(0,45,30,.25)] sm:w-64">
                <div className="aspect-[9/18] overflow-hidden rounded-[2.4rem] bg-white">
                  <div className="grid h-full place-items-center p-6 text-center">
                    <Logo className="h-24 w-24 object-contain" />
                    <div>
                      <p className="text-lg font-black text-[#06291f]">Ride Bangla App</p>
                      <p className="mt-2 text-xs leading-5 text-slate-500">Food · Grocery · Parcel · Courier</p>
                    </div>
                    <div className="w-full space-y-2">
                      {["Order now", "Track order", "Get support"].map((item) => (
                        <div key={item} className="rounded-xl bg-emerald-50 px-3 py-2 text-xs font-bold text-brand-green">{item}</div>
                      ))}
                    </div>
                  </div>
                </div>
            ...
            <div>
              <span className="text-[10px] font-extrabold uppercase tracking-[.18em] text-brand-green">Ride Bangla App</span>
              <h2 className="mt-4 text-4xl font-black tracking-[-.04em] text-[#06291f] sm:text-5xl">Your everyday services, <span className="text-brand-red">one tap away.</span></h2>
              <p className="mt-5 max-w-2xl text-base leading-8 text-slate-600">The app experience should feel as premium as the brand — simple navigation, clear service discovery, order tracking and support in one place.</p>
              <div className="mt-7 grid gap-3 sm:grid-cols-2">
                {["Fast service discovery", "Real-time order status", "Secure payment flow", "Dedicated support"].map((item) => (
                  <div key={item} className="flex items-center gap-2 rounded-xl bg-white px-3 py-3 text-xs font-bold text-slate-700 shadow-sm ring-1 ring-slate-100">
                    <CheckCircle2 className="h-4 w-4 text-brand-green" /> {item}
                  </div>
                ))}
              </div>
              <Link to="/apps" className="mt-7 inline-flex items-center gap-2 rounded-xl bg-[#06291f] px-5 py-3 text-sm font-extrabold text-white transition hover:bg-brand-green">View Apps <ArrowRight className="h-4 w-4" /></Link>
            </div>
          </div>
        </section>
            ...
            <div className="mb-8 flex items-end justify-between">
              ...
              <div className="hidden items-center gap-2 sm:flex">
                <button
                  ...
                  className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 bg-white text-slate-600 shadow-sm transition hover:bg-emerald-50 hover:text-brand-green"
                >
                  <ChevronLeft className="h-4 w-4" />
                </button>
                <button
            ...
            <div className="mt-6 flex items-center justify-center gap-2 sm:hidden">
              {testimonials.map((item, index) => (
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all ${
                    index === testimonialIndex ? "w-6 bg-brand-green" : "w-1.5 bg-slate-200"
                  }`}
                />
              ))}
            </div>
